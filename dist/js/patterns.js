...
// Scripts for pattern at ../source/patterns/02-components/advisor-tiles/05-responsive-grid.hbs
...
// Scripts for pattern at ../source/patterns/02-components/ratings/03-editable-stars.hbs
...
// Scripts for pattern at ../source/patterns/02-components/toolbar/01-toolbar.hbs


$(function(){

  var $window = $(window),      // most consistent GETTER for scrollTop
      $html = $('html,body'),   // most consistent SETTER for scrollTop
      minimumAffordance = 200,  // minimum pixels below toolbar allowed
      duration = 300;           // duration of animation in milliseconds

  $('.js-toolbar').each(function(){
    var $toolbar = $(this);

    $toolbar.on('click.vse.toolbar', 'a:not(.active)', function(){
      var windowHeight = $window.height(),
          scrollY = $window.scrollTop(),
          toolbarY = $toolbar.offset().top,
          toolbarHeight = $toolbar.height();
      if (windowHeight - (toolbarY + toolbarHeight - scrollY) < minimumAffordance) {
        $html.stop().animate({ scrollTop: toolbarY }, duration);
      }
    });
...
// Scripts for pattern at ../source/patterns/pages/01-home/_promo-pod.hbs
...
// Scripts for pattern at ../source/patterns/pages/11-account/_return-call.hbs

$(window).load(function(){
  var $pods = $('.return-call-settings-box');
  $pods.leveller({
    cssSelector: '.js-return-call-settings-btn',
    cssProperty: 'margin-top'
  });
  $(window).resize($.debounce(function(){
    $pods.leveller();
